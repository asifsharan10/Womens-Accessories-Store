...
(function ($) {
  $(function () {
    ...
    $(document).on("keypress keydown keyup", "input, button, a", function (e) {
      if (
        ...
        ($("#loading-indicator").is(":visible") || isMultipleFire)
      ) {
        e.preventDefault();
        isMultipleFire = true;
        return false;
  ...
  if (
    ...
    (typeof app_config.allowed_tc === "string" ||
      app_config.allowed_tc instanceof String)
  ) {
    //app_config.allowed_tc = cb.decryptAllowedTestCard(app_config.allowed_tc);
  }

  if (
    typeof disableUrlErrorHandler === "undefined" ||
    disableUrlErrorHandler !== true
  ) {
    cb.urlErrorHandler();
  }
})(jQuery);
  ...
  $.ajax({
    ...
    success: function (data) {
      if (
        ...
        data.context.errorFound == 0 &&
        data.context.prospectId
      ) {
        if ($("input[name=prospectId]").length) {
          $("input[name=prospectId]").val(data.context.prospectId);
        }
      } else {
        $("form[name=checkout_form]").append(
          '<input type="hidden" name="altered_action" value="new_order" />'
        );
      }
    },
  });
...
window.addEventListener("popstate", function (event) {
  if (
    app_config.enable_browser_back_button &&
...
if (
  ...
  if (!app_config.enable_browser_back_button) {
    ...
    if (app_config.pageType == "checkoutPage") {
      history.forward();
    } else {
      history.pushState({}, "", "");
    }
